...
foreach ($accounts as $ac){
  if (
    $ac->vars['last_billing'] == '2025-12-08'
    && stripos($ac->vars['name'], 'COMMISSION') === false
    && stripos($ac->vars['name'], 'REVERT') === false
) {
  	echo $i.") ".$ac->vars['name']."<BR>";
    $i++;
  }
...
foreach($files as $file){
	$str_data = file_get_contents("fix/files/".$file.".txt");
