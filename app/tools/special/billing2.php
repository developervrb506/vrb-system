...
$accounts = get_all_pph_accounts_for_billing_test($run_date); 
...
//		print_r($counts[$ag_account]);
		...
		if(isset($counts[$ag_account]["phone"])){ echo "----> EXISTS ";}
		else { echo "----> NOT EXIST ";}
		...
		if($sport > 0 || $phone > 0 || $live > 0 || $props > 0){
			...
			if($agent ->vars["is_commission"]){
				...
				$af_account = get_pph_account_test($agent ->vars["commission_owner"]);
