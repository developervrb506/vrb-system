...
function file_get_html_parts($start,$part,$url, $use_include_path = false, $context=null, $offset = -1, $maxLen=-1, $lowercase = true, $forceTagsClosed=true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT)
{
    // We DO force the tags to be terminated.
    $dom = new simple_html_dom(null, $lowercase, $forceTagsClosed, $target_charset, $stripRN, $defaultBRText, $defaultSpanText);
    // For sourceforge users: uncomment the next line and comment the retreive_url_contents line 2 lines down if it is not already done.
    ...
    // When $part = 0 , the  $start will be used as the cant of lines to be rested to the end of the file. 
	if (!$part){
	$total_string = $part; 	
    $x= ((strlen($contents)-$start)) ;
	$contents = substr($contents,$x,(strlen($contents)));	
	}
	else{	
	$total_string = round(strlen($contents)/$part);
	$contents = substr($contents,$start,(strlen($contents)-$total_string));	
	}
