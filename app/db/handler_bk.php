...
function check_login($email, $pass){
	...
	$login_sql_res  = mysql_query($login_sql) or die(mysql_error());
	if($login_info = mysql_fetch_array($login_sql_res)) {
...
function get_custom_campaigns_by_affiliate($aid){
	affiliates_db();
	$campaignes = array();
	$selections_sql = "SELECT * FROM custom_campaign WHERE affiliate = '$aid' AND deleted = 0 ORDER BY id DESC";
...
function insert_campaigne($camp){
	affiliates_db();
	$update = "INSERT INTO campaignes (name,campaignes.desc,url,id_sportsbook, affiliate)
				 VALUES('". $camp->name ."','". $camp->desc ."','". $camp->url ."', ". $camp->book->id .", ". $camp->affiliate .")";
	$res = mysql_query($update) or die(mysql_error());
}
...
function insert_affiliate($affiliate) {
   ...
   $add_affiliate = "INSERT INTO affiliates (password, nepass, firstname, lastname, email, city, address, country, state, zipcode, phone, registered, websitename, websiteurl, isadmin, sub, newsletter) VALUES
					('".md5($password)."','".two_way_encript($password)."','".$firstname."','".$lastname."','".$email."','".$city."','".$address."','".$country."',
					 '".$state."','".$zipcode."','".$phone."',now(),'".$websitename."' ,'".$websiteurl."',0,$sub,'".$newsletter."')";				
									  
   $add_affiliate_res = mysql_query($add_affiliate) or die(mysql_error());
...
function update_affiliate($affiliate) {
   ...
   $update_affiliate_res = mysql_query($update_affiliate) or die(mysql_error());
...
function check_email($email,$id) {
	...
	if($info = mysql_fetch_array($sql_res)) {
		$cant = $info['cant']; 
	}
...
function get_all_sportsbooks(){
	affiliates_db();
	$sportsbooks = array();
	$selections_sql = "SELECT * FROM sportsbooks ORDER BY id DESC";
	$selections_sql_res = mysql_query($selections_sql);
	while($selections_info = mysql_fetch_array($selections_sql_res)) {
		$id     = $selections_info['id'];
		$name   = $selections_info['name'];		
...
function get_affiliate_sportsbooks($id_aff){
	...
	$selections_sql = "SELECT s.id, s.name, s.url FROM sportsbooks as s, affiliates_by_sportsbook as abs
						 ...
						 AND abs.idbook = s.id AND abs.affiliatecode IS NOT NULL ORDER BY id ASC";
	$selections_sql_res = mysql_query($selections_sql);
	while($selections_info = mysql_fetch_array($selections_sql_res)) {
		$id     = $selections_info['id'];
		$name   = $selections_info['name'];		
...
function insert_books_affiliate($aff_id,$books) {
   ...
   foreach ($books as $idbook){  
	   ...
	   $add_record_res = mysql_query($add_record) or die(mysql_error());				 
	}	 
    return;   
}
...
function update_endorsement($affid,$id_book,$end,$end_signature) {
   affiliates_db();
   $exist = false;
	...
	if($exist){
		$update = "UPDATE endorsements
              SET 	endorsement  = '$end', endorsement_signature = '$end_signature'
			  WHERE id_book = '$id_book' and
			       	id_affiliate = '$affid'";
   		$update_res = mysql_query($update) or die(mysql_error());
	}else{
		$update = "INSERT INTO endorsements (id_book, id_affiliate, endorsement, endorsement_signature)
				 VALUES($id_book, $affid, '$end', '$end_signature')";
		$res = mysql_query($update) or die(mysql_error());
	} 
...
function get_endorsement($affid, $bookid){
	affiliates_db();
	$endorsement = array();
	$selections_sql = "SELECT endorsement, endorsement_signature, endorsement_image FROM endorsements WHERE id_book = '$bookid' AND id_affiliate = '$affid'";
	$selections_sql_res = mysql_query($selections_sql);
	if($selections_info = mysql_fetch_array($selections_sql_res)) {
...
function count_signups_by_agent($aid, $book, $from, $to,$webs){
	affiliates_db();
	$num = 0;
	$selections_sql = "SELECT COUNT(*) as num FROM signups as s, promotypes as p, campaignes as c WHERE s.affiliate IN ($webs)
