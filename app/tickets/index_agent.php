...
<? if (isset($mobile) and $mobile == 1) { ?>
    ...
    validations.push({id:"player",type:"null", msg:"Please choose a Player"});
    </script>
    <script type="text/javascript">
    ...
    <form method="post" action="actions/create_player.php" id="frm_2" onSubmit="return validate(validations)">
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="5">
       ...
       <tr >
            <td><div class="labels_tds">Player:</div></td>
            <td>
        
               <?
               $players = json_decode(@file_get_contents("http://www.sportsbettingonline.ag/utilities/process/reports/print_agent_players.php?agent=".two_way_enc($account,true)));
               if (!is_null($players)){
              ...
              <? foreach ($players as $p) {  ?>
                  ...
                  <option value="<? echo $p->vars->Player; ?>"><? echo $p->vars->Player; ?></option>
              <? } ?>      
              </select>
               
               
               <a id="a_player" style="" title="Click to sent a Message to a Player ">* PLAYER</a>
               <?
...
<? } else { ?>
    ...
    <span class="title">Create New Ticket </span>
    <form method="post" action="actions/create_agent.php" id="frm_2" onSubmit="return validate(validations)">
    <input name="cat" type="hidden" id="cat" value="<? echo $_GET["cat"] ?>">
    <input name="web" type="hidden" id="web" value="<? echo $web ?>">
    <input name="wpx" type="hidden" id="wpx" value="<? echo $account ?>">
    <table width="100%" border="0" cellspacing="0" cellpadding="5">
