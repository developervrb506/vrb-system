...
$ticket->update(array("pending_answer","completed"));
...
if($ticket->vars["ticket_category"] != 45 && $ticket->vars["ticket_category"] != 47){
  ...
  $res = $ticket->insert_response(clean_str_ck($_POST["message"]),  $_POST["acc"]);	
  $ticket->vars["pread"] = 0;
  $ticket->update(array("pread"));
}

if($ticket->vars["related_ticket"] > 0 &&  $ticket->vars["ticket_category"] != 45){
