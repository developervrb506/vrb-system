...
// Admin Access to Frank and Fabiola
if($current_clerk->vars["id"] == "54" || $current_clerk->vars["id"] == "2" ){$current_clerk->vars["super_admin"] = 1;}
$from = post_get("from");
...
if(!$current_clerk->vars["super_admin"]){
	...
	$john_clerks = array(60,105,104,110,135,111,136,128,152,46,23); 
...
<br /><br />
<div align="right">
<a href="sbo_matching_prepaid_transactions.php" class="normal_link" >Matching Transactions</a>
&nbsp;&nbsp;|&nbsp;&nbsp;
&nbsp;&nbsp;|&nbsp;&nbsp;
...
Method: 
<? 
echo file_get_contents("http://$cash_url/utilities/process/reports/print_payment_methods.php?filter=$all_methods&en=_search&ao=1&cm=$method"); 
?>
...
$trans = search_my_prepaid_transfers($from, $to, $status, $clerk,$method); ?>
<br /><br />
  ...
  <tr title="<? echo $tran->vars["comments"]; ?>" style="color:<? echo $tran->get_color() ?>; <? if($tran->vars["processor_status"] != "pe"){echo 'font-weight:bold;';} ?>" id="tr_<? echo $tran->vars["id"]; ?>">
    ...
    <td class="table_td<? echo $style ?>" align="center">
        ...
        <? if($tran->vars["processor_status"] != "de"){ ?>
            ...
            echo file_get_contents("http://$cash_url/utilities/process/reports/print_payment_methods.php?filter=$mfilter&en=_".$tran->vars["id"]); 	
            ?>
            <br />
...
function change_prepaid_status(id, value){	
	...
	types['de'] = "Denied";
	types['pe'] = "Pending";
	if(confirm("Are you sure you want to change the status to "+types[value]+"?")){
		var back_msg = document.getElementById("back_msg_"+id).value;
		
		if(back_msg != "" || value != 'de'){
			...
			alert("You need to write a reason to Deny");	
		}
	}
}
