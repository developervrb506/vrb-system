...
<? if(isset($metatag_id)){ ?>
    ...
    <div style="float:right;"><a href="<? echo $_SERVER['HTTP_REFERER']; ?>"><< Back</a></div>
    <br />    
...
<span class="page_title"><? echo 'Search Results' ?></span>
<div style="float:right;"><a href="<? echo $_SERVER['HTTP_REFERER']; ?>"><< Back</a></div>
<?
$count_metas = count(search_metatag($_POST["search_url"])); 
$metatags = search_metatag($_POST["search_url"]);
if ($count_metas > 0) {
?>
<form action="" method="post">  
  <table width="100" border="0" cellspacing="0" cellpadding="0" style="padding:10px;">
    <input name="search" type="hidden" id="search" value="Y"> 
    <tr>
      <td><strong>URL:</strong></td>
      <td><input name="search_url" type="text" id="search_url" size="100" value=""></td>
...
<table style="cursor:pointer;" class="sortable">
    ...
    <tbody id="the-list"> 
        ...
        <tr>
          ...
          <th class="table_td<? echo $style ?>" valign="top" style="text-align: center"><input style="cursor:pointer;" onClick="change_page('metatags.php?edit_id=<? echo $metatag->vars["id"] ?>')" type="button" name="Button" value="View" /></th>
          <th class="table_td<? echo $style ?>" valign="top" style="text-align: center"><input style="cursor:pointer;" onClick="delete_metatag('<? echo $metatag->vars["id"] ?>')" type="button" name="Button" value="Delete" /></th> 
        </tr>     
        <tr>
  ...
  <ul class="tsc_pagination tsc_paginationC tsc_paginationC01">
    ...
    <? for($i=0;$i<$paginationCount;$i++){ ?>    
        <li id="<? echo $i ?>_no" class="link">
          <a href="javascript:void(0);" onclick="changePagination('<? echo $i ?>','<? echo $i ?>_no')">
              <? echo ($i+1) ?>
          </a>
        </li>
    <? } ?>
    ...
    <li class="last link" id="last">
       <a href="javascript:void(0);" onclick="changePagination('<? echo ($paginationCount-1) ?>','last')">L a s t</a>
    </li>
    <li class="flash"></li>
  </ul>
