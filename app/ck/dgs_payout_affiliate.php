...
$agent_balance = file_get_contents("http://$cash_url/utilities/process/reports/vrb_sbo_agent_real_balance.php?agent=$account&byname=1");
if(!is_numeric($agent_balance)){$agent_balance = 0;}
...
function prevalidate(validations){
	if(validate(validations)){
		total_amount = <? echo $amount ?> + parseInt(document.getElementById("fees").value);
		if(total_amount > <? echo $agent_balance ?>){
			good = confirm("The amount is bigger than the player Blance ($<? echo $agent_balance ?>). Are you sure you want to insert this Payout?");
		}
	}else{
		good = false;
	}
...
<?php /*?><div class="form_box">
	<? echo file_get_contents("http://www.sportsbettingonline.ag/utilities/process/reports/print_player_note.php?player=$account&num=4"); ?>
</div><?php */?>
<div class="form_box">
...
<form method="post" onsubmit="return prevalidate(validations)" action="process/actions/dgs_payout_affiliate_action.php">
...
<div class="form_box">
    ...
    <br />
    <iframe src="http://<? echo $cash_url ?>/utilities/process/reports/last_payout_agnet_report.php?agent=<? echo $account ?>" frameborder="0" scrolling="auto" width="100%" height="50"></iframe>
</div>
</form>
</div>
