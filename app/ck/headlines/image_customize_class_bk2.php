...
/*
  ...
  */
  ...
  function image_base($data){
   ...
   foreach ($this->headlines as $hd) {
    ...
    or die("Cannot Initialize new GD image stream");
     ...
     imagepng($this->imgMain,  $base_path);

     // Free up memory for the original image
   ...
   if($this->control){ 
    ...
    // Load the second image you want to add
     ...
     $secondWidth = imagesx($secondImage);
     $secondHeight = imagesy($secondImage);
    ...
    imagecopy($this->imgMain, $secondImage, $x, $y, 0, 0, $secondWidth, $secondHeight);

   
    // Save the final image on the server
    imagepng($this->imgMain, $base_path2);
    ...
    $resizedImage = imagecreatefrompng($banner);
    ...
    imagecopy($this->imgMain, $resizedImage, $x, $y, 0, 0, $resizedWidth, $resizedHeight);
    ...
    $resizedImage = imagecreatefrompng($table_teams);
    ...
    $resizedImage = imagecreatefrompng($join_button);
    ...
    $resizedImage = imagecreatefrompng($img_away);
    ...
    // Load the image you want to resize and add to coordinates (229, 114)
    $resizedImage = imagecreatefrompng($img_home);
  ...
  //  imagettftext($this->imgMain, $fontSize, 0, $textX, $textY, $textColor, $fontFile, $data['away_name']);
...
// Specify the filename to save the final image
  ...
  $resizedImage = imagecreatefrompng($tv);
