...
//$search_criterias = "&payment_status=".$payment_status."&status=".$status."&completed=".$completed."&id_list=".$id_list."&search=".$search."&show_all=".$show_all;

$search_criterias = "&id_list=".$id_list."&search=".$search."&show_all=".$show_all;

?>
<div class="page_content" style="padding-left:10px;" id="container" class='table-container'>
<? $data = "&cid=".$current_clerk->vars['id']."&clerk=".str_replace(" ","_",$current_clerk->vars['name'])."&pg=tn"; ?>
<? echo file_get_contents("https://www.rescuecenter.com/utilities/ui/touchnote-entries/list.php?p=LPasjuY65FTq3Qpld1sadm0O0I8".$_SERVER['QUERY_STRING'].$data.$search_criterias.$pagenumber); ?>
