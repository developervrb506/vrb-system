...
switch($method){
	case "MG":
		$ptransaction = get_processing_transaction($tid);
		$ddesc = "MONEYGRAM / " . $ptransaction["mtcn"] . " / " . $ptransaction["city"];
		$dref = "SENDER: " . $ptransaction["sender_name"];
		$fdesc = "MG PAYOUT FEES";
	break;
	case "WU":
		$ptransaction = get_processing_transaction($tid);
		$ddesc = "WESTERNUNION / " . $ptransaction["mtcn"] . " / " . $ptransaction["city"];
		$dref = "SENDER: " . $ptransaction["sender_name"];
	...
	break;
	case "LOCAL":
		$ddesc = "LOCAL CASH PAYOUT";
		$fdesc = "LOCAL CASH PAYOUT FEES";
	break;	
	case "WIRE":
		$ddesc = "BANK WIRE PAYOUT";
		$fdesc = "BANK WIRE PAYOUT FEES";
	break;	
	case "PAYPAL":
		$ddesc = "PAYPAL PAYOUT";
	...
	break;
	case "RE":
		$mp = get_moneypak_transaction(str_replace("RE","",$tid));
...
echo do_post_request("http://$cash_url/utilities/process/reports/process_dgs_affiliate_payout.php",$data);
