...
if(isset($_POST["process"])){
	if(isset($_POST["update_id"])){		
		...
		if($upagn->vars["inserted"]){
			...
			if($original_amount != $upagn->vars["amount"]){	
				$params = "?transaction=".two_way_enc(mt_rand)."&ip=".two_way_enc($upagn->vars["amount"]).
					"&cache=".two_way_enc($upagn->vars["dgs_dID"]);
			...
			if($original_fees != $upagn->vars["fees"]){
				$params = "?transaction=".two_way_enc(mt_rand)."&ip=".two_way_enc($upagn->vars["fees"]).
			...
			//bonus
			if($original_bonus != $upagn->vars["bonus"]){	
				$bonus_amount = ($upagn->vars["amount"] * $upagn->vars["bonus"]) / 100;
				$params = "?transaction=".two_way_enc(mt_rand)."&ip=".two_way_enc($bonus_amount).
					"&cache=".two_way_enc($upagn->vars["dgs_bID"]);
				$params .= "&mxs=".two_way_enc("AMOUNT CHANGED");
				$upagn->vars["dgs_bID"] = 
					file_get_contents("http://www.sportsbettingonline.ag/utilities/process/reports/change_amount_dgs_transaction.php$params");
			}
	...
	$upagn = get_special_deposit($_GET["sd"]);	
	...
	if($_GET["st"] == "de"){	
		//dgs Cancel
		$params = "?transaction=".two_way_enc(mt_rand)."&dk=".two_way_enc("DENIED.")."&od=1&cache=".two_way_enc($upagn->vars["dgs_dID"]);
		file_get_contents("http://www.sportsbettingonline.ag/utilities/process/reports/cancel_dgs_transaction.php$params");	
		
		if($upagn->vars["dgs_fID"] > 0){
			$params = "?transaction=".two_way_enc(mt_rand)."&dk=".two_way_enc("DENIED.")."&od=1&cache=".two_way_enc($upagn->vars["dgs_fID"]);
			file_get_contents("http://www.sportsbettingonline.ag/utilities/process/reports/cancel_dgs_transaction.php$params");	
		}
		if($upagn->vars["dgs_bID"] > 0){
			$params = "?transaction=".two_way_enc(mt_rand)."&dk=".two_way_enc("DENIED.")."&od=1&cache=".two_way_enc($upagn->vars["dgs_bID"]);
			file_get_contents("http://www.sportsbettingonline.ag/utilities/process/reports/cancel_dgs_transaction.php$params");	
		}
	}
	
	update_money_order_status($upagn->vars["moid"],$_GET["st"]);
}
?>
...
if(isset($_GET["detail"])){
	//details
	$deposit = get_special_deposit($_GET["dep"]);
	if(is_null($deposit)){
		...
		$new = false;
		$hidden = '<input name="update_id" type="hidden" id="update_id" value="'.$deposit->vars["id"] .'" />';
	}
	?>
	...
	validations.push({id:"bonus",type:"numeric", msg:"Bonus is required"});
	validations.push({id:"special_method",type:"null", msg:"Method is required"});
    </script>
	<div class="form_box" style="width:500px;">
        <form method="post" action="special_deposit.php?e=64" onsubmit="return validate(validations)">
        <input name="process" type="hidden" id="process" value="1" />
		<? echo $hidden; ?>
        <table width="100%" border="0" cellspacing="0" cellpadding="10">        
          <tr>
            <td>Player</td>
            <td><input name="player" type="text" id="player" value="<? echo $deposit->vars["player"] ?>" /></td>
          </tr> 
          <tr>
            <td>Amount</td>
            <td><input name="amount" type="text" id="amount" value="<? echo $deposit->vars["amount"] ?>" /></td>
          </tr> 
          <tr>
            <td>Fees</td>
            <td><input name="fees" type="text" id="fees" value="<? echo $deposit->vars["fees"] ?>" /></td>
          </tr> 
          <tr>
            <td>Bonus (%)</td>
            <td><input name="bonus" type="text" id="bonus" value="<? echo $deposit->vars["bonus"] ?>" /></td>
          </tr>  
          <tr>
            <td>Method</td>
            <td>
            	<?
				$select_option = true;
				$current_method = $deposit->vars["method"]->vars["id"];
          ...
          <tr>
            <td>Date</td>
            <td><input type="text" value="<? echo $deposit->vars["ddate"] ?>" readonly="readonly" /></td>
          </tr> 
          <tr>
            <td>Inserted</td>
            <td><? echo $deposit->str_inserted() ?></td>
          </tr> 
          <? } ?>  
          <tr>
            <td>Comment</td>
            <td><textarea name="comment" cols="35" rows="7" id="comment"><? echo $deposit->vars["comment"] ?></textarea></td>
          </tr>
          <? if(!$new){ ?>
          <tr>
            <td>Admin Comment</td>
            <td><textarea name="back" cols="35" rows="7" id="back"><? echo $deposit->vars["feedback"] ?></textarea></td>
          </tr>
			<?php /*?>  <? if($current_clerk->im_allow("special_deposits_process") && $deposit->vars["status"] == "pe"){ ?>
              <tr>
                <td>Status</td>
                <td>
                    <select name="status" id="status">
                      <option value="pe">Pending</option>
                      <option value="ac" <? if($deposit->vars["status"] == "ac"){echo 'selected="selected"';} ?>>Accepted</option>
                      <option value="de" <? if($deposit->vars["status"] == "de"){echo 'selected="selected"';} ?>>Denied</option>
                    </select>
                </td>
              </tr> 
              <? } ?><?php */?>
	...
	//list
	?>
    <span class="page_title">Special Deposits</span><br /><br />
    <a href="?detail" class="normal_link">+ Add Deposit</a>
    <br /><br />
    <?
	$from = $_POST["datef"]; 
	...
	?>
    <form action="special_deposit.php" method="post">
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td class="table_header" align="center">Bonus</td>
      ...
      <tr>
        ...
        <td class="table_td<? echo $style ?>" align="center">$<? echo $mt->vars["bonus"]; ?></td>
        ...
        <td class="table_td<? echo $style ?>" align="center">
            ...
            &nbsp;|&nbsp;
            <a href="javascript:;" onclick="if(confirm('Are you sure you want to Accept this Deposit?')){location.href = 'special_deposit.php?sd=<? echo $mt->vars["id"]; ?>&st=ac';}" class="normal_link">
            	Accept
            </a>
            &nbsp;|&nbsp;
        	<a href="javascript:;" onclick="if(confirm('Are you sure you want to Deny this Deposit?')){location.href = 'special_deposit.php?sd=<? echo $mt->vars["id"]; ?>&st=de';}" class="normal_link">
            	Deny
            </a><? } ?>
        </td>
        <? } ?>
