...
echo "---------------<BR>";
echo "NHL SCHEDULE   <br>";
echo "---------------<BR><BR>";
// THIS JOBS WORKS FOR UPDATE THE ESPNID ALSO FOR THE WINNER TEAM - IT WORKS FOR NEW SEASON ONLY UPDATE THE SEASON

$teams = get_nhl_teams("espn_short");
$season = "21-22";
$espn_id = get_all_nhl_espn_id($season);

echo "<pre>";
$j= 0;
foreach ($teams as $t){ $j++;
  $schedule = get_nhl_schedule($t->vars["espn_short"],$teams);	
  ...
  if(!empty($schedule)){
	  ...
	  foreach ($schedule as $sh){
		...
		if(!isset($espn_id[trim($sh["espn_id"])])){
		  ...
		  $game = get_nhl_schedule_team($t->vars["id"],$season,trim($sh["date"]));
		  if(is_null($game)){
			$game = new _nhl_games(); 
