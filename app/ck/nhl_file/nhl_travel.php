...
<img src="images/NHL.png" style="width:60px; margin-bottom:-20px"><span class="page_title">&nbsp;&nbsp;&nbsp;TEAMS TRAVEL INFO</span><br /><br /><br />
<?	

if($current_clerk->vars['id'] != 86 ){
	$subject = 'NHL FILE ACCESS';
...
$season = $_POST["season"];
$teams = get_nhl_teams("id");
$i=0;

if($team != ""){
  $dist_team = get_nhl_all_teams_distance();	
} else{
  $four = $five = $six = $seven = $eigth = $nine = $ten = $game = 1;	
   $first = $second = 0;
}
...
<form id="frm_dist" method="post">
 Season:&nbsp;
  <select name="season">
  <option <? if ($season == '19-20'){ echo ' selected="selected" ';}?>  value="21-22" >21-22</option>   
...
&nbsp;&nbsp;&nbsp;&nbsp;
 ...
 <?php 
 // NHL DOES NOT HAVE OTHER LINES ONLY GAME
...
<? if($team != "") { ?>
<?
 $schedule =  get_nhl_schedule_by_team($team,$season);
 $total_sch = count($schedule);
 $rotations = get_awayrotations_between_dates('nhl',$schedule[0]->vars["date"],$schedule[$total_sch-1]->vars["date"]);
 $lines =  get_league_lines_between_dates('NHL',$schedule[0]->vars["date"],$schedule[$total_sch-1]->vars["date"]);
echo "<pre>";
//  print_r($rotations);
// print_r($lines);
echo "</pre>";
...
<table id="tbl_sch" border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <td name="lines" class="table_header second" align="center" width="25px"></td> <?php */?>    
 ...
 foreach ($schedule as $sc){  if($i % 2){
   ...
   <tr>
     ...
     <td class="table_td<? echo $style ?>" ><img style="width:25px" src="http://localhost:8080/ck/nhl_file/images/<? echo $teams[$team2]->vars["logo"]?>.png">
	...
	/*?> <td  name ="lines" class="table_td<? echo $style ?>_yellow first" align="center"><? echo $sc->vars["first_away_points"]." - ".$sc->vars["first_home_points"] ?></td>     
     ...
     <td name ="lines"class="table_td<? echo $style ?>_blue second" align="center"><? if($total) { echo check_line($str_team, $sc->vars["second_away_points"],$sc->vars["second_home_points"],$total,'total'); } ?></td>  <?php */?>
     ...
     <? /// 3 GAMES ?>
	 <? $days = 3 ; ?>  
     <td class="table_td<? echo $style ?>" align="center" id="three1_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_nhl_team_travel_info($schedule,$dist_team,$team,$i,$days,'restdays');}?></td>  
     <? $away_days = get_nhl_team_travel_info($schedule,$dist_team,$team,$i,$days,'away') ?> 
     <td class="table_td<? echo $style ?>" align="center" id="three2_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo $away_days;}?></td>               
   	 <td class="table_td<? echo $style ?>" align="center" id="three3_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_nhl_team_travel_info($schedule,$dist_team,$team,$i,$days,'miles');}?></td>               
     <? if ($away_days >= $days){?><script>color_row('three','<? echo $i?>','<? echo $style ?>');</script><? } ?>        					   

    <? /// 4 GAMES ?>
  
	 <? $days = 4 ;  ?>  
     <td  class="table_td<? echo $style ?> four" align="center" id="four1_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_nhl_team_travel_info($schedule,$dist_team,$team,$i,$days,'restdays');}?></td>  
     <? $away_days = get_nhl_team_travel_info($schedule,$dist_team,$team,$i,$days,'away') ?> 
     <td  class="table_td<? echo $style ?> four" align="center" id="four2_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo $away_days;}?></td>               
   	 <td  class="table_td<? echo $style ?> four" align="center" id="four3_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_nhl_team_travel_info($schedule,$dist_team,$team,$i,$days,'miles');}?></td>               
     <? if ($away_days >= $days){?><script>color_row('four','<? echo $i?>','<? echo $style ?>');</script><? } ?>        					   
  
     
     <? /// 5 GAMES ?>
	 <? $days = 5 ; ?>  
     <td class="table_td<? echo $style ?> five" align="center" id="five1_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_nhl_team_travel_info($schedule,$dist_team,$team,$i,$days,'restdays');}?></td>  
     <? $away_days = get_nhl_team_travel_info($schedule,$dist_team,$team,$i,$days,'away') ?> 
     <td class="table_td<? echo $style ?> five" align="center" id="five2_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo $away_days;}?></td>               
   	 <td class="table_td<? echo $style ?> five" align="center" id="five3_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_nhl_team_travel_info($schedule,$dist_team,$team,$i,$days,'miles');}?></td>               
     <? if ($away_days >= $days){?><script>color_row('five','<? echo $i?>','<? echo $style ?>');</script><? } ?>        					   

     <? /// 6 GAMES ?>
	 <? $days = 6 ; ?>  
     <td class="table_td<? echo $style ?> six" align="center" id="six1_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_nhl_team_travel_info($schedule,$dist_team,$team,$i,$days,'restdays');}?></td>  
     <? $away_days = get_nhl_team_travel_info($schedule,$dist_team,$team,$i,$days,'away') ?> 
     <td class="table_td<? echo $style ?> six" align="center" id="six2_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo $away_days;}?></td>               
   	 <td class="table_td<? echo $style ?> six" align="center" id="six3_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_nhl_team_travel_info($schedule,$dist_team,$team,$i,$days,'miles');}?></td>               
     <? if ($away_days >= $days){?><script>color_row('six','<? echo $i?>','<? echo $style ?>');</script><? } ?>        					   

     <? /// 7 GAMES ?>
	 <? $days = 7 ; ?>  
     <td class="table_td<? echo $style ?> seven" align="center" id="seven1_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_nhl_team_travel_info($schedule,$dist_team,$team,$i,$days,'restdays');}?></td>  
     <? $away_days = get_nhl_team_travel_info($schedule,$dist_team,$team,$i,$days,'away') ?> 
     <td class="table_td<? echo $style ?> seven" align="center" id="seven2_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo $away_days;}?></td>               
   	 <td class="table_td<? echo $style ?> seven" align="center" id="seven3_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_nhl_team_travel_info($schedule,$dist_team,$team,$i,$days,'miles');}?></td>               
     <? if ($away_days >= $days){?><script>color_row('seven','<? echo $i?>','<? echo $style ?>');</script><? } ?>        					   

     <? /// 8 GAMES ?>
	 <? $days = 8 ; ?>  
     <td class="table_td<? echo $style ?> eigth" align="center" id="eigth1_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_nhl_team_travel_info($schedule,$dist_team,$team,$i,$days,'restdays');}?></td>  
     <? $away_days = get_nhl_team_travel_info($schedule,$dist_team,$team,$i,$days,'away') ?> 
     <td class="table_td<? echo $style ?> eigth" align="center" id="eigth2_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo $away_days;}?></td>               
   	 <td class="table_td<? echo $style ?> eigth" align="center" id="eigth3_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_nhl_team_travel_info($schedule,$dist_team,$team,$i,$days,'miles');}?></td>               
     <? if ($away_days >= $days){?><script>color_row('eigth','<? echo $i?>','<? echo $style ?>');</script><? } ?>        					   

     <? /// 9 GAMES ?>
	 <? $days = 9 ; ?>  
     <td class="table_td<? echo $style ?> nine" align="center" id="nine1_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_nhl_team_travel_info($schedule,$dist_team,$team,$i,$days,'restdays');}?></td>  
     <? $away_days = get_nhl_team_travel_info($schedule,$dist_team,$team,$i,$days,'away') ?> 
     <td class="table_td<? echo $style ?> nine" align="center" id="nine2_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo $away_days;}?></td>               
   	 <td class="table_td<? echo $style ?> nine" align="center" id="nine3_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_nhl_team_travel_info($schedule,$dist_team,$team,$i,$days,'miles');}?></td>               
     <? if ($away_days >= $days){?><script>color_row('nine','<? echo $i?>','<? echo $style ?>');</script><? } ?>        					   
     
     <? /// 10 GAMES ?>
	 <? $days = 10 ; ?>  
     <td class="table_td<? echo $style ?> ten" align="center" id="ten1_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_nhl_team_travel_info($schedule,$dist_team,$team,$i,$days,'restdays');}?></td>  
     <? $away_days = get_nhl_team_travel_info($schedule,$dist_team,$team,$i,$days,'away') ?> 
     <td class="table_td<? echo $style ?> ten" align="center" id="ten2_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo $away_days;}?></td>               
   	 <td class="table_td<? echo $style ?> ten" align="center" id="ten3_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_nhl_team_travel_info($schedule,$dist_team,$team,$i,$days,'miles');}?></td>               
     <? if ($away_days >= $days){?><script>color_row('ten','<? echo $i?>','<? echo $style ?>');</script><? } ?>        					   
     
     <? if($other) { ?>
     <? /// Other GAMES ?>
	 <? $days = $other ; ?>  
     <td class="table_td<? echo $style ?> other" align="center" id="other1_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_nhl_team_travel_info($schedule,$dist_team,$team,$i,$days,'restdays');}?></td>  
     <? $away_days = get_nhl_team_travel_info($schedule,$dist_team,$team,$i,$days,'away') ?> 
     <td class="table_td<? echo $style ?> other" align="center" id="other2_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo $away_days;}?></td>               
   	 <td class="table_td<? echo $style ?> other" align="center" id="other3_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_nhl_team_travel_info($schedule,$dist_team,$team,$i,$days,'miles');}?></td>               
