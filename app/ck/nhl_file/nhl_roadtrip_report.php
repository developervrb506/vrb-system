...
<script type="text/javascript" src="http://localhost:8080/ck/nhl_file/js/functions.js"> </script>
...
<img src="images/NHL.png" style="width:60px; margin-bottom:-20px"><span class="page_title">&nbsp;&nbsp;&nbsp;TEAMS TRAVEL REPORT</span><br />
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span> Show the information for the next Game after the # of Away Days </span><br /><br />
<?	

if($current_clerk->vars['id'] != 86 ){
	$subject = 'NHL REPORT ACCESS 2';
...
if ($str_teams == "" && $other != "" ){?>
 ...
 foreach($selected_teams as $steam){
	 $st = trim($steam);
	 $i=0;
	 $schedule =  get_nhl_schedule_by_team($st,$season);
   ...
   <div id="div_team_<? echo $teams[$st]->vars["id"] ?>"> 
    <div>
       <img style="width:35px" src="http://localhost:8080/ck/nhl_file/images/<? echo $teams[$st]->vars["logo"]?>.png">
	...
	<table id="tbl_sch" border="0" cellspacing="0" cellpadding="0">
	  <tr>
		...
		<?php /*?><td name="lines" class="table_header first" align="center" width="80px">1st H Result</td> 
	 ...
	 foreach ($schedule as $sc){  if($i % 2){
		  ...
		  $days = $other ;  
		  $rest_days = get_nhl_team_travel_info($schedule,$dist_team,$st,$i,$days,'restdays');
		  $away_days = get_nhl_team_travel_info($schedule,$dist_team,$st,$i,$days,'away') ;
	  ...
	  if($print_line){
	   ...
	   <tr>
		...
		<?php /*?> <td  name ="lines" class="table_td<? echo $style ?>_yellow first" align="center"><? echo $sc->vars["first_away_points"]." - ".$sc->vars["first_home_points"] ?></td>     
		 <? $rot_key = $sc->vars["date"]."_".$teams[$sc->vars["team_away"]]->vars["team_id"];?>
		 <? $rot = $rotations[$rot_key]["awayrotationnumber"]; ?>
		 ...
		 <td name ="lines"class="table_td<? echo $style ?>_blue second" align="center"><? if($total) { $result = check_line($str_team, $sc->vars["second_away_points"],$sc->vars["second_home_points"],$total,'total'); $status = $result[0]; $result[0] = ""; if($status == '+'){ $s_total++; }if($status == '-'){ $s_l_total++; } if($status == '='){ $s_p_total++; }  echo $result;  } ?></td>  <?php */?>
		 ...
		 <td class="table_td<? echo $style ?> other" align="center" id="other3_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_nhl_team_travel_info($schedule,$dist_team,$st,$i,$days,'miles');}?></td>               
      ...
      <tr>
		...
		<?php /*?><td class="table_td<? echo $style ?>_yellow first " align="center"></td>        
