...
<?
//Page Logic
$clean_message = str_replace("'","",preg_replace( "/\r/", "",preg_replace( "/\n/", "______", param('message',false))));
if (isset($_POST["message"])) { 

	if (!isset($_POST["edit"])) {
		$message = new _agent_messages();
		
		if ( param('agent') != "" && !is_null(param('agent')) ){
		...
		$message->vars["miss_agents"] = param('exclude');		
		
		$message->vars["message"] = $clean_message;
  ...
  <div id="div_table" class="default-hidden" <? if($edit){ echo 'style="display:none"';}?> >
   ...
   ?>
      <tr id="tr_<? echo $msj->vars["id"] ?>">
        ...
        <th class="table_td<? echo $style ?>" align="justify"><? echo str_replace("______","<br />",$msj->vars["message"]); ?></th>
    ...
    <form action="" method="post" target="_self" onsubmit="" >
      ...
      <table width="50%" border="0" cellspacing="0" cellpadding="5">
        <tr>
          ...
          <td valign="top"><select name="agent_type" id="agent_type" onchange="change_agent_type(this.value);" style="height:22px;">
          ...
          <td colspan="2" valign="top"><div class="default-hidden" id="text_agent">
              <input name="agent" type="text" id="agent" value="<? echo $message->vars["agent"] ?>" size="15" />
            </div></td>            
        </tr>
        <tr>
          <td>Exclude Agents:</td>
          <td colspan="5"><input type="text" name="exclude" id="exclude" value="<? echo $message->vars["miss_agents"] ?>" placeholder="Separate by coma" /></td>
        </tr>
        <tr>
          <td valign="top">Expiration Date:</td>
          <td><input required name="expiration_date" type="text" id="expiration_date" value="<? echo $expiration_date ?>" size="11" /></td>
          <td valign="top"><select required name="hour" id="hour" style="height:21px;">
          ...
          <td valign="top"><select required name="minute" id="minute" style="height:21px;">
        ...
        <tr>
          <td>Message:</td>
          <td colspan="5"><textarea required name="message" id="message" rows="5" cols="21"><? echo str_replace("______","\n",$msj->vars["message"]); ?></textarea></td>
        </tr>
        <tr>
          <td>Allow agent to hide this message:</td>
