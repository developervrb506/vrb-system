...
//////
////      THIS JOB IS NOT RUNNING on 2025, baseball file changed. now games_job.php is the main job
...
// To check weekly if some pitcher was missed it runs all Mondays at 6:40 am.
 ...
 if (date('D') == 'Tue'){
	 if ($time > date('H:i',strtotime('06:25 AM')) && $time < date('H:i',strtotime('07:00 AM'))){ 
        include("pitchers_bats_throws.php");
	   	include("pitchers_missed_info.php"); 
	}
 }
...
if (isset($today_games[0]->vars["id"])) { 
	...
	include("pitchers_velocity_away.php");
	include("pitchers_velocity_home.php");
	}
	
	if ($time > date('H:i',strtotime('07:05 AM')) && $time < date('H:i',strtotime('07:20 AM'))){
	include("pitchers_data_away.php");
	include("baseball_alerts.php");
	include("parkfactor.php");
	include("team_speeds.php");
	}
	if ($time > date('H:i',strtotime('07:30 AM')) && $time < date('H:i',strtotime('07:55 AM'))){
	include("pitchers_data_home.php");
	include("pitchers_velocity_away.php");
	}
	if ($time > date('H:i',strtotime('08:05 AM')) && $time < date('H:i',strtotime('08:25 AM'))){
	include("pitchers_stadistics.php");
	include("pitchers_velocity_home.php");
	include("pitchers_groundball.php?t=away");
	include("pitchers_groundball.php?t=home");

	}	
	if ($time > date('H:i',strtotime('08:30 AM')) && $time < date('H:i',strtotime('08:55 AM'))){
	include("pitchers_groundball.php?t=away");
	include("pitchers_groundball.php?t=home");
	}
	if ($time > date('H:i',strtotime('09:05 AM')) && $time < date('H:i',strtotime('09:25 AM'))){
	include("team_bullpen.php");
	}	
