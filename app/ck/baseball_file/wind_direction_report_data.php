   ...
   <Br/ >
    ...
    <option value= "">Select One to Filter </option>
     <? foreach ($wind as $_wind) { ?>
       <option <? if ($_POST["wind"] == $_wind["id"]) { echo ' selected ';} ?> value= "<? echo $_wind["id"] ?>"><? echo $_wind["description"]?> ( <? echo $_wind["direction"] ?>)</option>
     <? } ?>
     </select>
    <BR><BR>
    Runs:
...
<BR />
 ...
 <? //if (isset($_POST["stadium"])){ ?>
    <? 
      ...
      //if ($_POST["stadium"] > 0) { ?>
      <?  print_wind_direction($from,$to,$_POST["stadium"],$indoors,$_POST["wind"],$runs); ?>
	 ...
	 /* else {
        ...
        <span><strong>Stadium:  </strong> <? echo strtoupper($_stadium->vars["name"]) ?></span><BR><BR>
				  <?  
 		 $data = print_wind_direction($from,$to,$_stadium->vars["team_id"],$indoors,$_POST["wind"],$runs);
