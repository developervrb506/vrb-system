...
$year = date('Y',strtotime($game->vars["startdate"]));	
 ...
 $home_team = get_baseball_team($game->vars["team_home"]); 
   $away_team = get_baseball_team($game->vars["team_away"]); 
      ...
      $html2 = file_get_html("http://mlb.mlb.com/mlb/gameday/index.jsp?gid=".$umpire_date."_".$away_team->vars["mlb_nick"]."_".$home_team->vars["mlb_nick"]."_".$game->vars["game_number"]."&mode=plays");
	
    echo $game->vars["id"]." - http://mlb.mlb.com/mlb/gameday/index.jsp?gid=".$umpire_date."_".$away_team->vars["mlb_nick"]."_".$home_team->vars["mlb_nick"]."_".$game->vars["game_number"]."&mode=plays";
	
       foreach ( $html2->find('div.results') as $div ) {
...
$sql="SELECT *
FROM `game`
WHERE (pitcher_away = '' || pitcher_home = '')
AND startdate > '2013-01-01 00:00:00'
AND startdate < '2013-07-14 00:00:00'
and postponed != 1
ORDER BY startdate
limit 100
";	
