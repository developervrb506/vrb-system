...
<?
 ...
 foreach($games as $game){
	...
	// Air Density
     ...
     $air_density = $game->get_air_density($game->get_pascals_from_inches($weather->vars["air_pressure"]),$temp_kelvin);
	 $dewpoint_celsius = $game->get_celsius_temp($weather->vars["dewpoint"]);
	 $water_vapour = $game->get_water_vapour($dewpoint_celsius);
	 $moist_air_density = $game->get_moist_air_density ($game->get_kelvin_temp($weather->vars["temp"]),$weather->vars["humidity"],$water_vapour,$game->get_pascals_from_inches($weather->vars["air_pressure"]));
...
function get_without_pk(){
baseball_db();	
$sql = "SELECT * FROM `game` WHERE postponed = 0 and pk=-99 limit 10";
