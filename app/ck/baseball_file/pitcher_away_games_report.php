...
?>
  ...
  <span class="page_title">Pitcher Away Report for last 2 Games
  </span><br /><br />
  ...
  <form method="post">
    ...
    <BR><BR>
     Team 1:
     <? 
       create_objects_list("team1", "team1", $stadiums, "team_id", "team_name", "Select Team",$_POST["team1"],$on_change,"_baseball_stadium");  ?>
       
      Team 2:
     <? 
       create_objects_list("team2", "team2", $stadiums, "team_id", "team_name", "Select Team",$_POST["team2"],$on_change,"_baseball_stadium");  ?>  
    ...
    Less Than  : 
      <input  style="width:50px" name="rest" type="text" id="test" value="<? echo $_POST["rest"] ?>" /> Rest Days. 
       ...
       &nbsp;&nbsp;&nbsp;&nbsp;
      <input  type="checkbox" <? if (!isset($_POST['team1'])){ echo 'checked="checked"'; } if ($_POST['indoors']){ echo 'checked="checked"'; } ?>  value="1"  name="indoors" />
...
if (isset($_POST["from"])){ 
  ...
  $next_to = date( "Y-m-d", strtotime( "15 day", strtotime(date($to)))); 
  $games_home = get_baseball_games_home_team($from,$to,$_POST["team1"],$_POST['indoors']);
 ...
 if (!empty($games_home)) { 
   ...
   $j=0;
	  foreach ($pitchers as $_pitchers){
	   ...
	   if (!is_null($game)){
		 if  ($game->vars["team_home"] == $_POST["team2"]){
 ...
 if (!empty($all_games)) {  
  foreach ($all_games as $_game_home){
	 if  ($_game_home->vars["pitcher"] != $_game_home->vars["pitcher_home"]){
	 
	  if ($_POST["indoors"]==1){
	   if ($_game_home->vars["real_roof_open"] == 1){
		   $pitchers_2[$_game_home->vars["pitcher_away"]]["game"] = $_game_home->vars["id"];
		   $pitchers_2[$_game_home->vars["pitcher_away"]]["pitcher"] = $_game_home->vars["pitcher_away"];
		   $pitchers_2[$_game_home->vars["pitcher_away"]]["startdate"] = $_game_home->vars["startdate"];
		}
	  } else {
		   $pitchers_2[$_game_home->vars["pitcher_away"]]["game"] = $_game_home->vars["id"];
		   $pitchers_2[$_game_home->vars["pitcher_away"]]["pitcher"] = $_game_home->vars["pitcher_away"];
  ...
  if (!is_null($pitchers_2)) {
	   foreach ($pitchers_2 as $_pitchers){
	   $game =  get_baseball_games_next_game_player($_pitchers["pitcher"],$_pitchers["startdate"]);	  
		   if (!is_null($game)){
			$game->vars["pitcher"] = $_pitchers["pitcher"];
			$all_games_2[$j] = $game; 
