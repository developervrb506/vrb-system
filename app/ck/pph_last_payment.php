...
if(isset($_GET["detail"])){
	...
	<div class="form_box" style="width:400px;">
        ...
        <form method="post" action="pph_last_payment.php?e=39&br=1" onsubmit="return validate(validations)">
    ...
    Show:
    	...
    	<select name="" onchange="location.href = 'pph_last_payment.php?br=1&show='+this.value;">
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td class="table_header" align="center">Last Payment</td>
      </tr>
      <?
	  $i=0;
	   ...
	   foreach($groups as $grp){
		   ...
		   foreach($grp as $acc){
			  ...
			  <tr>
               ...
               <? 
			   $from_report = date("Y-m-d",strtotime(date("Y-m-d")." -6 months"));
			   $to_report = date("Y-m-d");
			   $last_payment = last_payment_pph_transaction($from_report,$to_report,$acc->vars["id"]);
			   
			   if(!empty($last_payment)){
			    
			      echo "From: ".$last_payment->vars["from_account"]->vars["name"]." To: ".$last_payment->vars["to_account"]->vars["name"].", Amount: $".$last_payment->vars["amount"].", Date: ".$last_payment->vars["tdate"].", Note: ".nl2br($last_payment->vars["note"]);
