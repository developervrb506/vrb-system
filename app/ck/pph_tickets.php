...
if(isset($_POST["process"])){
	if(isset($_POST["update_id"])){
		$upacc = get_pph_ticket($_POST["update_id"]);
		$upacc->vars["agent"] = $_POST["agent"];
		$upacc->vars["issue"] = $_POST["issue"];
		$upacc->vars["solution"] = $_POST["solution"];
		$upacc->vars["resolved"] = $_POST["resolved"];
		if($upacc->vars["close_clerk"] == "" || $upacc->vars["close_clerk"] == 0){
			$upacc->vars["close_clerk"] = $current_clerk->vars["id"];
		}
		$upacc->update();
	}else{
		$newacc = new _pph_ticket();
		$newacc->vars["agent"] = $_POST["agent"];
		$newacc->vars["issue"] = $_POST["issue"];
		$newacc->vars["solution"] = $_POST["solution"];
		$newacc->vars["resolved"] = $_POST["resolved"];
		...
		$newacc->insert();
		header("Location: pph_tickets.php?e=62");		
	}
}
?>
...
if(isset($_GET["detail"])){
	//details
	$ticket = get_pph_ticket($_GET["tid"]);
	if(is_null($ticket)){
	...
	<div class="form_box" style="width:700px;">
        <form method="post" action="pph_tickets.php?e=62" onsubmit="return validate(validations)">
	...
	//list
	?>
    <span class="page_title">Pending PPH tickets</span><br /><br />
    <a href="?detail" class="normal_link">+ New Ticket</a>&nbsp;&nbsp;&nbsp;&nbsp;
    <a href="pph_tickets_report.php" class="normal_link">Report</a>
    <br /><br />
	<? include "includes/print_error.php" ?>  
    <? $tickets = search_pph_tickets("", "", "0", ""); ?>
    <? if(count($tickets)>0){ ?>  
