...
echo "---------------<BR>";
echo "MLB SCHEDULE   <br>";
echo "---------------<BR><BR>";
// THIS JOBS WORKS FOR UPDATE THE ESPNID ALSO FOR THE WINNER TEAM - IT WORKS FOR NEW SEASON ONLY UPDATE THE SEASON

$teams = get_mlb_teams("espn_short");
$season =  date("Y");
$espn_id = get_all_mlb_espn_id($season);

echo "<pre>";
$j= 0;
foreach ($teams as $t){ $j++;
  $schedule = get_mlb_schedule($t->vars["espn_short"],$teams);	
  ...
  if(!empty($schedule)){
	  ...
	  foreach ($schedule as $sh){
		...
		if(!isset($espn_id[trim($sh["espn_id"])])){
		  ...
		  $game = get_mlb_schedule_team($t->vars["id"],$season,trim($sh["date"]));
		  if(is_null($game)){
			$game = new _mlb_games(); 
			...
			$game->insert();	// CHECK THE MONTH DATE ON FUNCTION get_mlb_schedule 
