...
<script type="text/javascript" src="http://localhost:8080/process/js/FileSaver.js"> </script>
<script type="text/javascript" src="http://localhost:8080/ck/mlb_file/js/functions.js"> </script>
...
<img src="images/MLB.png" style="width:60px; margin-bottom:-20px"><span class="page_title">TEAMS TRAVEL REPORT</span><br />
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span> Show the information for the next Game after the # of Away Days </span><br /><br />
<?	

if($current_clerk->vars['id'] != 86 ){
	$subject = 'MLB REPORT ACCESS 2';
...
<BR><BR>
 ...
 #Away: &nbsp;&nbsp;  <input  style="width:35px" type="number" name="away" id="away" value="<? echo $p_away?>">&nbsp;
 <BR><BR>
...
<td>
 ...
 <tr>
 <td>  <input name="first" id="first" type="checkbox" <? if($first) { echo ' checked="checked" ' ;}?>  value="1" onchange="show_hide_column( 'tbl_sch', 'first');"> 1st 5 Innings </td>
 </tr>
</table>
</td>
 <td>
...
if ($str_teams == "" && $other != "" ){?>
 ...
 foreach($selected_teams as $steam){
	 $st = trim($steam);
	 $i=0;
	 $schedule =  get_mlb_schedule_by_team($st,$season);
  ...
  <div id="div_team_<? echo $teams[$st]->vars["id"] ?>">
    <div>
       <img style="width:35px" src="http://localhost:8080/ck/mlb_file/images/<? echo $teams[$st]->vars["logo"]?>.png">
	...
	<table id="tbl_sch" border="0" cellspacing="0" cellpadding="0">
	  <tr>
		...
		<td name="lines" class="table_header first" align="center" width="80px">1st 5 Innings H Result</td> 
		<td name="lines" class="table_header first" align="center" width="80px">1st 5 Innings H Spread</td>     
		<td name="lines" class="table_header first" align="center" width="25px"></td>     
		<td name="lines" class="table_header first" align="center" width="80px">1st 5 Innings H MONEY</td>     
		<td name="lines" class="table_header first" align="center" width="25px"></td>         
		<td name="lines" class="table_header first" align="center" width="80px">1st 5 Innings H TOTAL</td>     
		<td name="lines" class="table_header first" align="center" width="25px"></td>     
	 ...
	 foreach ($schedule as $sc){  if($i % 2){
		  ...
		  $days = $other ;  
		  $rest_days = get_mlb_team_travel_info($schedule,$dist_team,$st,$i,$days,'restdays');
		  $away_days = get_mlb_team_travel_info($schedule,$dist_team,$st,$i,$days,'away') ;
		//   echo $rest_days." --$i- ".$away_days."<BR>";
		  
	  if($sc->vars["team_home"] != $st){ $print_line = false; } // To include only Home Games
	  ...
	  if($print_line){
	   ...
	   <tr>
		 ...
		 <td name ="lines"class="table_td<? echo $style ?>_yellow first" align="center"><? if($spread) { $result = check_line($str_team, $sc->vars["five_away_points"],$sc->vars["five_home_points"],$spread,'spread'); $status = $result[0]; $result[0] = ""; if($status == '+'){ $f_spread++; } if($status == '-'){ $f_l_spread++; } if($status == '='){ $f_p_spread++; }  echo $result;  } ?></td>   
		 <td name ="lines"class="table_td<? echo $style ?>_yellow first" align="center"><? echo $money ?></td>   
		 <td name ="lines"class="table_td<? echo $style ?>_yellow first" align="center"><? if($money) {$result = check_line($str_team, $sc->vars["five_away_points"],$sc->vars["five_home_points"],$spread,'money'); $status = $result[0]; $result[0] = ""; if($status == '+'){ $f_money++; } if($status == '-'){ $f_l_money++; } if($status == '='){ $f_p_money++;  } echo $result;  } ?></td>   
		 <td name ="lines"class="table_td<? echo $style ?>_yellow first" align="center"><? echo $total ?></td>   
		 <td name ="lines"class="table_td<? echo $style ?>_yellow first" align="center"><? if($total) { $result = check_line($str_team, $sc->vars["five_away_points"],$sc->vars["five_home_points"],$total,'total'); $status = $result[0]; $result[0] = ""; if($status == '+'){ $f_total++; } if($status == '-'){ $f_l_total++; } if($status == '='){ $f_p_total++; }  echo $result;  } ?></td>    
		 ...
		 <td class="table_td<? echo $style ?> other" align="center" id="other3_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_mlb_team_travel_info($schedule,$dist_team,$st,$i,$days,'miles');}?></td>               
	  ...
	  <?  } 
       ...
       if($p_away == $away_days){$print_line = true;} else {$print_line = false;}
	     $i++; 
	   }
