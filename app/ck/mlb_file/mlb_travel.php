...
<img src="images/MLB.png" style="width:60px; margin-bottom:-20px"><span class="page_title">TEAMS TRAVEL INFO</span><br /><br /><br />
<?	

if($current_clerk->vars['id'] != 86 ){
	$subject = 'MLB FILE ACCESS';
...
$season = $_POST["season"];
$teams = get_mlb_teams("id");
$i=0;

if($team != ""){
  $dist_team = get_mlb_all_teams_distance();	
} else{
...
<form id="frm_dist" method="post">
 Season:&nbsp;
  <select name="season">
  <option <? if ($season == '2017'){ echo ' selected="selected" ';}?>  value="2017" >2017</option>    
  <option <? if ($season == '2016'){ echo ' selected="selected" ';}?>  value="2016" >2016</option>  
  <option <? if ($season == '2015'){ echo ' selected="selected" ';}?>  value="2015" >2015</option>
  <option <? if ($season == '2014'){ echo ' selected="selected" ';}?>  value="2014" >2014</option>
 </select>
  &nbsp;&nbsp;
 ...
 GAMES:
 <table style="float:right; margin-right:2760px; margin-top:-40px">
 <tr>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
 ...
 <table style="float:right; margin-right:2620px; margin-top:-90px">
 <tr>
 ...
 <tr>
 <td>  <input name="first" id="first" type="checkbox" <? if($first) { echo ' checked="checked" ' ;}?>  value="1" onchange="show_hide_column( 'tbl_sch', '1st 5 Innings');"> 1st 5 Innings </td>
 </tr>
...
<? if($team != "") { ?>
<?
 $schedule =  get_mlb_schedule_by_team($team,$season);
 $total_sch = count($schedule);
 $rotations = get_awayrotations_between_dates('mlb',$schedule[0]->vars["date"],$schedule[$total_sch-1]->vars["date"]);
 $lines =  get_league_lines_between_dates('MLB',$schedule[0]->vars["date"],$schedule[$total_sch-1]->vars["date"]);
echo "<pre>";
//  print_r($rotations);
// print_r($lines);
echo "</pre>";
...
<table id="tbl_sch" border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <td name="lines" class="table_header first" align="center" width="80px">1st 5 Innings Result</td> 
    <td name="lines" class="table_header first" align="center" width="80px">1st 5 Innings Spread</td>     
    <td name="lines" class="table_header first" align="center" width="25px"></td>     
    <td name="lines" class="table_header first" align="center" width="80px">1st 5 Innings MONEY</td>     
    <td name="lines" class="table_header first" align="center" width="25px"></td>         
    <td name="lines" class="table_header first" align="center" width="80px">1st 5 Innings TOTAL</td>     
    <td name="lines" class="table_header first" align="center" width="25px"></td>     
 ...
 foreach ($schedule as $sc){  if($i % 2){
   ...
   <tr>
     ...
     <td class="table_td<? echo $style ?>" ><img style="width:25px" src="http://localhost:8080/ck/mlb_file/images/<? echo $teams[$team2]->vars["logo"]?>.png">
     ...
     <td  name ="lines" class="table_td<? echo $style ?>_yellow first" align="center"><? echo $sc->vars["five_away_points"]." - ".$sc->vars["five_home_points"] ?></td>     
     <? $rot_key = $sc->vars["date"]."_".$teams[$sc->vars["team_away"]]->vars["team_id"];?>
     <? $rot = $rotations[$rot_key]["awayrotationnumber"]; ?>
     <? $line_key = $sc->vars["date"]."_1st 5 Innings_".$rot;?>
     <? $spread = ""; $spread = $lines[$line_key][$str_team."_spread"]; ?>
     <? $money = ""; $money = $lines[$line_key][$str_team."_money"]; ?>
     <? $total = ""; $total = $lines[$line_key][$str_team."_total"]; ?>          
     <td name ="lines"class="table_td<? echo $style ?>_yellow first" align="center"><? echo $spread ?></td>   
     <td name ="lines"class="table_td<? echo $style ?>_yellow first" align="center"><? if($spread) { echo check_line($str_team, $sc->vars["five_away_points"],$sc->vars["five_home_points"],$spread,'spread'); } ?></td>   
     <td name ="lines"class="table_td<? echo $style ?>_yellow first" align="center"><? echo $money ?></td>   
     <td name ="lines"class="table_td<? echo $style ?>_yellow first" align="center"><? if($money) { echo check_line($str_team, $sc->vars["five_away_points"],$sc->vars["five_home_points"],$spread,'money'); } ?></td>   
     <td name ="lines"class="table_td<? echo $style ?>_yellow first" align="center"><? echo $total ?></td>   
     <td name ="lines"class="table_td<? echo $style ?>_yellow first" align="center"><? if($total) { echo check_line($str_team, $sc->vars["five_away_points"],$sc->vars["five_home_points"],$total,'total'); } ?></td>    
     ...
     <? /// 3 GAMES ?>
	 <? $days = 3 ; ?>  
     <td class="table_td<? echo $style ?>" align="center" id="three1_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_mlb_team_travel_info($schedule,$dist_team,$team,$i,$days,'restdays');}?></td>  
     <? $away_days = get_mlb_team_travel_info($schedule,$dist_team,$team,$i,$days,'away') ?> 
     <td class="table_td<? echo $style ?>" align="center" id="three2_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo $away_days;}?></td>               
   	 <td class="table_td<? echo $style ?>" align="center" id="three3_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_mlb_team_travel_info($schedule,$dist_team,$team,$i,$days,'miles');}?></td>               
     <? if ($away_days >= $days){?><script>color_row('three','<? echo $i?>','<? echo $style ?>');</script><? } ?>        					   

    <? /// 4 GAMES ?>
  
	 <? $days = 4 ;  ?>  
     <td  class="table_td<? echo $style ?> four" align="center" id="four1_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_mlb_team_travel_info($schedule,$dist_team,$team,$i,$days,'restdays');}?></td>  
     <? $away_days = get_mlb_team_travel_info($schedule,$dist_team,$team,$i,$days,'away') ?> 
     <td  class="table_td<? echo $style ?> four" align="center" id="four2_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo $away_days;}?></td>               
   	 <td  class="table_td<? echo $style ?> four" align="center" id="four3_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_mlb_team_travel_info($schedule,$dist_team,$team,$i,$days,'miles');}?></td>               
     <? if ($away_days >= $days){?><script>color_row('four','<? echo $i?>','<? echo $style ?>');</script><? } ?>        					   
  
     
     <? /// 5 GAMES ?>
	 <? $days = 5 ; ?>  
     <td class="table_td<? echo $style ?> five" align="center" id="five1_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_mlb_team_travel_info($schedule,$dist_team,$team,$i,$days,'restdays');}?></td>  
     <? $away_days = get_mlb_team_travel_info($schedule,$dist_team,$team,$i,$days,'away') ?> 
     <td class="table_td<? echo $style ?> five" align="center" id="five2_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo $away_days;}?></td>               
   	 <td class="table_td<? echo $style ?> five" align="center" id="five3_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_mlb_team_travel_info($schedule,$dist_team,$team,$i,$days,'miles');}?></td>               
     <? if ($away_days >= $days){?><script>color_row('five','<? echo $i?>','<? echo $style ?>');</script><? } ?>        					   

     <? /// 6 GAMES ?>
	 <? $days = 6 ; ?>  
     <td class="table_td<? echo $style ?> six" align="center" id="six1_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_mlb_team_travel_info($schedule,$dist_team,$team,$i,$days,'restdays');}?></td>  
     <? $away_days = get_mlb_team_travel_info($schedule,$dist_team,$team,$i,$days,'away') ?> 
     <td class="table_td<? echo $style ?> six" align="center" id="six2_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo $away_days;}?></td>               
   	 <td class="table_td<? echo $style ?> six" align="center" id="six3_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_mlb_team_travel_info($schedule,$dist_team,$team,$i,$days,'miles');}?></td>               
     <? if ($away_days >= $days){?><script>color_row('six','<? echo $i?>','<? echo $style ?>');</script><? } ?>        					   

     <? /// 7 GAMES ?>
	 <? $days = 7 ; ?>  
     <td class="table_td<? echo $style ?> seven" align="center" id="seven1_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_mlb_team_travel_info($schedule,$dist_team,$team,$i,$days,'restdays');}?></td>  
     <? $away_days = get_mlb_team_travel_info($schedule,$dist_team,$team,$i,$days,'away') ?> 
     <td class="table_td<? echo $style ?> seven" align="center" id="seven2_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo $away_days;}?></td>               
   	 <td class="table_td<? echo $style ?> seven" align="center" id="seven3_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_mlb_team_travel_info($schedule,$dist_team,$team,$i,$days,'miles');}?></td>               
     <? if ($away_days >= $days){?><script>color_row('seven','<? echo $i?>','<? echo $style ?>');</script><? } ?>        					   

     <? /// 8 GAMES ?>
	 <? $days = 8 ; ?>  
     <td class="table_td<? echo $style ?> eigth" align="center" id="eigth1_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_mlb_team_travel_info($schedule,$dist_team,$team,$i,$days,'restdays');}?></td>  
     <? $away_days = get_mlb_team_travel_info($schedule,$dist_team,$team,$i,$days,'away') ?> 
     <td class="table_td<? echo $style ?> eigth" align="center" id="eigth2_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo $away_days;}?></td>               
   	 <td class="table_td<? echo $style ?> eigth" align="center" id="eigth3_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_mlb_team_travel_info($schedule,$dist_team,$team,$i,$days,'miles');}?></td>               
     <? if ($away_days >= $days){?><script>color_row('eigth','<? echo $i?>','<? echo $style ?>');</script><? } ?>        					   

     <? /// 9 GAMES ?>
	 <? $days = 9 ; ?>  
     <td class="table_td<? echo $style ?> nine" align="center" id="nine1_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_mlb_team_travel_info($schedule,$dist_team,$team,$i,$days,'restdays');}?></td>  
     <? $away_days = get_mlb_team_travel_info($schedule,$dist_team,$team,$i,$days,'away') ?> 
     <td class="table_td<? echo $style ?> nine" align="center" id="nine2_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo $away_days;}?></td>               
   	 <td class="table_td<? echo $style ?> nine" align="center" id="nine3_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_mlb_team_travel_info($schedule,$dist_team,$team,$i,$days,'miles');}?></td>               
     <? if ($away_days >= $days){?><script>color_row('nine','<? echo $i?>','<? echo $style ?>');</script><? } ?>        					   
     
     <? /// 10 GAMES ?>
	 <? $days = 10 ; ?>  
     <td class="table_td<? echo $style ?> ten" align="center" id="ten1_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_mlb_team_travel_info($schedule,$dist_team,$team,$i,$days,'restdays');}?></td>  
     <? $away_days = get_mlb_team_travel_info($schedule,$dist_team,$team,$i,$days,'away') ?> 
     <td class="table_td<? echo $style ?> ten" align="center" id="ten2_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo $away_days;}?></td>               
   	 <td class="table_td<? echo $style ?> ten" align="center" id="ten3_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_mlb_team_travel_info($schedule,$dist_team,$team,$i,$days,'miles');}?></td>               
     <? if ($away_days >= $days){?><script>color_row('ten','<? echo $i?>','<? echo $style ?>');</script><? } ?>        					   
     
     <? if($other) { ?>
     <? /// Other GAMES ?>
	 <? $days = $other ; ?>  
     <td class="table_td<? echo $style ?> other" align="center" id="other1_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_mlb_team_travel_info($schedule,$dist_team,$team,$i,$days,'restdays');}?></td>  
     <? $away_days = get_mlb_team_travel_info($schedule,$dist_team,$team,$i,$days,'away') ?> 
     <td class="table_td<? echo $style ?> other" align="center" id="other2_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo $away_days;}?></td>               
   	 <td class="table_td<? echo $style ?> other" align="center" id="other3_<? echo $i ?>"><? if($i<$days-1){ echo "-";} else {echo get_mlb_team_travel_info($schedule,$dist_team,$team,$i,$days,'miles');}?></td>               
