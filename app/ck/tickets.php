...
foreach ($live_help_departments as $lhdep) {    
 ...
 $i=0;
   ...
   if($current_clerk->im_allow("all_tickets")){
    ...
    $tickets = search_tickets(date("Y-m-d",strtotime(date("Y-m-d")." - 1 month")), "", 1, "","agents",$livechat_dept_ids,"",$current_clerk->vars["id"],$removed,$cat_db,$acc_db,$p_answer,false,$items_per_page,($page_index*$items_per_page),"");	
	$count_tickets = search_tickets(date("Y-m-d",strtotime(date("Y-m-d")." - 1 month")), "", 1, "","agents",$livechat_dept_ids,"",$current_clerk->vars["id"],$removed,$cat_db,$acc_db,$p_answer,true,0,0,"");
...
&nbsp;&nbsp;
<input type="checkbox" name="p_answer" value="1" <? if ($p_answer == 1){ echo ' checked="checked" '; } ?> >&nbsp;<strong>Pending Answer</strong>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
   <? if($current_clerk->im_allow("delete_tickets")  || $current_clerk->admin()){ ?>
		   ...
		   if (!isset($all_clerks[$responses[count($responses)-1]->vars["by"]])){
			  if (contains_ck($style,"_red")){
				$style = str_replace("_red","_yellow",$style);
				 $tk->vars["pending_answer"] = 1;
				 $tk->vars["completed"] = 0;	
				 $tk->update(array("pending_answer","completed"));  
			  }
			  else {
				  $style .= "_yellow";
			     $tk->vars["pending_answer"] = 1;
  ...
  <tr  <?  if(!$current_clerk->im_allow("delete_tickets")){ ?>title="<? echo small_text(strip_tags($tk->vars["message"]),150); ?>" <? } ?> id="tr_<? echo $tk->vars["id"]; ?>">
     ...
     <? $allow = false;
	 if($current_clerk->im_allow("delete_tickets")  || $current_clerk->admin()){ $allow = true ?>
    ...
    <td  <?  if($current_clerk->im_allow("delete_tickets")  || $current_clerk->admin()){ ?>title="<? echo small_text(strip_tags($tk->vars["message"]),150); ?>" <? } ?>  class="table_td<? echo $style ?>" align="center"><? echo $tk->vars["name"]; ?><br />
