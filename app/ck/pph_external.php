...
<? if($current_clerk->im_allow("external_pph_billing") || true){ ?>
<?
if(isset($_GET["change_home"])){
	$upagn = get_pph_account_external($_GET["aid"]);
...
if(isset($_POST["process"])){
	if(isset($_POST["update_id"])){
		$upagn = get_pph_account_external($_POST["update_id"]);
		...
		$newagn = new _pph_account_external();
...
if(isset($_GET["detail"])){
	//details
	$agent = get_pph_account_external($_GET["acc"]);
	if(is_null($agent)){
	...
	<div class="form_box" style="width:400px;">
        ...
        <form method="post" action="pph_external.php?e=39&br=1" onsubmit="return validate(validations)">
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="10">
          ...
          <tr>
            <td>Master Agent</td>
            <? $all_agents = get_all_pph_accounts_external(); ?>
          ...
          <tr <? if(!$agent ->vars["is_commission"]){ ?> style="display:none" <? } ?> id="commrow">
            ...
            <? $nc_agents = search_pph_accounts_external("","0","0"); ?>
    ...
    <p>
		...
		&nbsp;&nbsp;|&nbsp;&nbsp;
		<a href="pph_bill_report_external.php" class="normal_link">Billing Report</a>
		&nbsp;&nbsp;|&nbsp;&nbsp;
		<a href="pph_current_bill_report_external.php" class="normal_link">Current Billing Report</a>
    </p>
    ...
    Show:
    	...
    	<select name="" onchange="location.href = 'pph_external.php?br=1&show='+this.value;">
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
	  ...
	  $i=0;
	   ...
	   switch($show_type){ 
		  case "a":
			  $agents = search_pph_accounts_external("","0");
		  break;
		  case "l":
			  $agents = search_pph_accounts_external();
		  break;
		  case "d":
			  $agents = search_pph_accounts_external("","1");
		  break;
		  default :
			  $agents = search_pph_accounts_external(1,"0");
		  break;
	  }
		   ...
		   foreach($grp as $acc){
			  ...
			  <tr>
				...
				<td class="table_td<? echo $style ?>" align="center"> 
					<?php /*?><input name="" type="checkbox" value="1" <? if($acc ->vars["house"]){ ?> checked="checked" <? } ?> onclick='document.getElementById("ichanger").src = "pph_external.php?change_home="+(this.checked*1)+"&aid=<? echo $acc->vars["id"]; ?>";' /><?php */?>
				...
				<td class="table_td<? echo $style ?>" align="center">
					...
					<? if($acc ->vars["deleted"]){ ?>
						...
						<a href="javascript:;" onclick="if(confirm('Are you sure you want to undelete this account?')){location.href='process/actions/delete_pph_account_external.php?ac=<? echo $acc->vars["id"]; ?>'}" class="normal_link">UnDelete</a>
						
					<? }else{ ?>
					
						<a href="javascript:;" onclick="if(confirm('Are you sure you want to delete this account?')){location.href='process/actions/delete_pph_account_external.php?ac=<? echo $acc->vars["id"]; ?>'}" class="normal_link">Delete</a>
