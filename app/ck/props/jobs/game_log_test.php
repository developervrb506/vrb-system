...
function get_espn_games_custom_test($league,$date,$field1,$value1,$field2,$value2){
	sbo_sports_db();
	...
	$sql = "SELECT  * from espn_games where  league = '".$league."'and  espn_id <> '-1' and  espn_id =  401437914    ORDER BY RAND() LIMIT 0,3";
...
echo "<pre>" ; 
     ...
     //$leagues = array("nba","mlb","ncaaf","nfl");	   
    $leagues = array("mlb");	
    ...
    foreach ($leagues as $league){
		...
		//	exit;
			 //Game Data
			$games = get_espn_games_custom_test($league,$yesterday,"finished",1,"game_log",1);   	//GET ALL THE TODAY FINISHED GAMES WITH GAME LOG
			...
			$games = get_espn_games_custom_test($league,$yesterday,"finished",1,"finished",1);   	//GET ALL THE TODAY FINISHED GAMES WITHOUT GAME LOG
	   ...
	   case "mlb"; // check
		   ...
		   $yesterday = date( "Y-m-d", strtotime( "-1 day", strtotime(date( "Y-m-d")))); 
			    ...
			    // $game->vars["espn_id"] = 401356132  ;
				   ...
				   if(empty($game_players)){
					...
					//  print_r($game_log);
					 // exit;
	      				if($game_log){
						  $game->vars["game_log"] = 2; // TEMP WIHILE GAME LOGS ARE DONE//1; // Player Props
						  $game->update(array("game_log"));
						 // print_r($game);
						  echo "------>UPDATE	";
						}
			...
			if($completed){
			 ...
			 echo "<BR>------GAME DATA ------<BR>";
			$games = get_espn_games_custom($league,$yesterday,"finished",1,"game_log",3);   	//GET ALL THE TODAY FINISHED GAMES WITH GAME LOG
			print_r($games);
			if(!empty($games)){
