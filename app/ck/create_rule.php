      ...
      <tr>
       ...
       <td>  
		  ...
		  foreach ($groups as $group) { ?>
           ...
           <input  type="checkbox"   onclick="javascript:check_all('<? echo $group->vars["id"] ?>','<? echo count($clerks)?>')" /> <? echo $group->vars["name"]?>&nbsp;&nbsp;<a id="extend_<? echo $group->vars["id"] ?>" href="javascript:display_div('div_'+<? echo $group->vars["id"]?>)" onclick="colapse('extend_'+<? echo $group->vars["id"] ?>);" class="normal_link" title="See all clerks for this group" >+</a>
          
            <div id="div_<? echo $group->vars["id"]?>" style="display:none" >  
             <? foreach ($clerks as $_clerk){ $i++; ?>
              &nbsp;&nbsp;<input class="box_<? echo $group->vars["id"] ?>" name="clerk_<? echo $i ?>" value="<? echo $_clerk->vars["id"] ?>" id="clerk_<? echo $group->vars["id"] ?>"  type="checkbox"  /> <? echo $_clerk->vars["name"] ?><BR>
            <? } ?>
            </div>
          <? } ?>
          
           <BR/>
            <? $customer_service = get_all_clerks("","2");; ?>          
           <input  type="checkbox"  checked="checked"  onclick="javascript:check_all('0','<? echo count($customer_service)?>')" /> Clerks &nbsp;&nbsp;<a id="extend_0" href="javascript:display_div('div_0')" onclick="colapse('extend_0');" class="normal_link" title="See all clerks for this group" >+</a>
          
            <div id="div_0" style="display:none" >  
             <? foreach ($customer_service as $_customer_service){ $i++; ?>
              &nbsp;&nbsp;<input  checked="checked" class="box_0" name="clerk_<? echo $i ?>" value="<? echo $_customer_service->vars["id"] ?>" id="clerk_0"  type="checkbox"  /> <? echo $_customer_service->vars["name"] ?><BR>
            <? } ?>
            </div>
