...
class _dgs_robot{
	...
	function login(){
		...
		$this->getUrl("http://".$this->vars["url"]."/engine/Login.aspx", 'post', $data);
		$this->html = strip_tags($this->getUrl("http://".$this->vars["url"]."/engine/wager/CreateSports.aspx?WT=0"), "<input>");
	}
	...
	function create_bet2($pre_place = false){
		...
		$data = array('__VIEWSTATE'=>$code,'ctl00$WagerContent$btn_Continue_top'=>'Continue','lg_'.$this->vars["league"]=>$this->vars["league"]);
		$this->html = $this->getUrl("http://".$this->vars["url"]."/engine/wager/CreateSports.aspx?WT=0", 'post', $data);
		...
		if($pre_place){
			...
			$data = array('__VIEWSTATE'=>$code,'ctl00$WagerContent$btn_Continue0'=>'Continue','text_'.$linecode=>$this->vars["amount"]);
			$this->html = $this->getUrl("http://".$this->vars["url"]."/engine/wager/Schedule.aspx?WT=0&lg=".$this->vars["league"], 'post', $data);
		
		
			//step3
			$code = $this->get_value_by_id($this->html, "__VIEWSTATE");
			$gameid = $line[1]."_".$line[0];
			$data = array('__VIEWSTATE'=>$code/*,'BUY_1466990_0'=>'0'*/,
						'AMT_'.$gameid=>$this->vars["amount"],'UseSameAmount'=>'1',
						'ctl00$WagerContent$btn_Continue1'=>'Continue');
			$this->html = $this->getUrl("http://".$this->vars["url"]."/engine/wager/CreateWager.aspx?WT=0&lg=".$this->vars["league"]."&sel=".$linecode."_".$this->vars["amount"], 'post', $data);
		}
		
		return array("line"=>$line[2],"juice"=>$line[3]);
	}
	...
	function place_bet(){
		...
		$this->html = $this->getUrl("http://".$this->vars["url"]."/engine/wager/ConfirmWager.aspx?WT=0", 'post', $data);
		if(!contains_ck($this->html, "ctl00_WagerContent_ctl00_lblFollError")){$res = 1;}
	...
	function get_line_from_html2($html, $rotation, $type){
		switch($type){
		...
		for ($i = 0; $i < $items->length; $i++){
			if(substr($items->item($i)->nodeValue,-3) == $rotation){
				for($e = 2; $e < 7; $e++){				
	...
	function get_line_from_html($html, $rotation, $type){
		switch($type){
			...
			case "under": $num1 = "2"; $num2 = "3"; break;
			case "spread": $num1 = "0"; $num2 = "1"; break;	
		}
