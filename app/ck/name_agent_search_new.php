...
if(isset($_POST["search"])){
	...
	$first = trim(clean_get("first"));
	$last = trim(clean_get("last"));
	if($phone != "" || $email != "" || $first != "" || $last != ""){
		$names = _search_agent_name($current_clerk->vars["id"], $phone, $email, $first, $last);
	}else{
		$names = array();	
	}
}else{
...
&nbsp;&nbsp;&nbsp;
Name: <input name="first" type="text" id="first" value="<? echo $first ?>" />
&nbsp;&nbsp;&nbsp;
Last Name: <input name="last" type="text" id="last" value="<? echo $last ?>" />
&nbsp;&nbsp;&nbsp;
