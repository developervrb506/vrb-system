...
function get_nba_schedule($team1,$teams) { ;

$link = "http://www.espn.com/nba/team/schedule/_/name/".$teams[$team1]->vars["espn_short"]."/";
echo $link."<BR>";
       ...
       //  print_r($div); exit;
		 ...
		 foreach($div->find("table tr") as $tr) { // tr    
		    ...
		    if($k>=1){ // k
			    $line = true;
				$j=0;
                if (contains_ck($tr->plaintext,"Opponent")){  $line = false; }
				else {$line = true;}
				...
				foreach ($tr->find("td") as $td){  //td
				  $j++;
				   ...
				   if(in_array($td->plaintext,$months)){ $line = false; $month = $td->plaintext; echo "ENTRA ".$td->plaintext;}
					if($line){ //line
					//  echo $td->plaintext; exit;
					   //DATE
					   if($j==1){ // 1
						 //  echo strtoupper(substr($td->plaintext,5,3))." - ";
						   ...
						   if (!contains_ck($row,"Post") && !contains_ck($row,"PM")){
							   ...
							   //Getting ESPN ID
							    foreach($td->find('a') as $a2){  //a
								 ...
								 // $espn_id .= "*";
  							     // $espn_id = str_center('p://www.espn.com/nba/game?gameId=','*', $espn_id);
  							     // $espn_id = str_replace("d/","",$espn_id);
...
function get_nba_team_distance($link,$teams){
$distance = array();
...
function get_nba_scores($espn_id) { ;

$link = "http://www.espn.com/nba/boxscore?gameId=".$espn_id;
echo $link;
...
function get_nba_team_travel_info($schedule,$distance,$team,$index,$days,$action){
	//$schedule =  geb_nba_schedule_by_team($team,$season);
...
function get_vs_rest_days($team,$date){
	...
	$date_away = get_nba_last_team_game ($team,$date,"away");
	$date_home =  get_nba_last_team_game ($team,$date,"home");
