...
<img src="images/NBA.png" style="width:60px; margin-bottom:-20px"><span class="page_title">TEAMS DISTANCE</span><br /><br /><br />
<?	
$parsing = false; // PARSING ONLY IS TRUE the first time. or in a maitanance.

$team = param("team");
$teams = get_nba_teams("team");
$i=0;

if($team != ""){
  $dist_team = get_nba_teams_distance($team);	
}
if(isset($_POST["dist"])){
  ...
  for($k=1;$k<=30;$k++){
	if($k!= $team){
	  ...
	  if(!isset($dist_team[$team."_".$k])){
	   $distance = new _nba_teams_distance();
...
if($parsing){
 $t = get_nba_team($team);
 $distance = get_nba_team_distance($t->vars["map_link"],$teams);
}
?>
...
<? if($parsing){ ?> <td class="table_header" align="center">Miles </td><? } ?>
 ...
 <?  if($ts->vars["id"] != $team){  $i++; ?>
   <tr>
     ...
     <td class="table_td<? echo $style ?>" align="center"><img style="width:60px" src="http://localhost:8080/ck/nba_file/images/<? echo $ts->vars["logo"]?>.png"></td>     
