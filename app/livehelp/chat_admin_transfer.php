...
<form name="counter">
Auto transfer to <big><b><?php echo "$user[name]" ?></b></big> of <?php echo $department['name'] ?> in <input type="text" name="counter_value" value="" class="input" size=2> seconds...
<br><br>
...
<b><?php echo $user['name'] ?></b> is currently <b>OFFLINE</b>.
<br><br>
...
<?php
	for ( $c = 0; $c < count( $departments ); ++$c )
	{
		...
		print "
				...
				<thead>
				<tr>
					<th colspan=5>Department: <b>$department[name]</th>
				</tr>
				</thead>
				<tbody class=\"subhead\">
				<tr>
		...
		for ( $c2 = 0; $c2 < count( $department_users ); ++$c2 )
		{
			...
			if ( $user['userID'] != $session_chat[$sid]['admin_id'] )
				$transfer_button = "<input type=\"button\" class=\"go\" OnClick=\"location.href='chat_admin_transfer.php?sessionid=$sessionid&sid=$sid&requestid=$requestid&userid=$user[userID]&action=transfer_verify&deptid=$department[deptID]'\" value=\"Transfer\">" ;
