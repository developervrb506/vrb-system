		...
		if ( $dbh[ 'ok' ] )
		{
			...
			for ( $c = 0; $c < count( $sessions ); ++$c )
			{
				...
				if ( file_exists( "$DOCUMENT_ROOT/web/chatsessions/$session[sessionID]"."_transcript.txt" ) && file_exists( "$DOCUMENT_ROOT/web/chatsessions/$session[sessionID]"."_transcript_info.txt" ) )
				{
					...
					LIST( $userid, $from_screen_name, $email, $deptid, $aspid, $phone ) = explode( "<:>", $transcript_info ) ;
					
					ServiceTranscripts_put_ChatTranscript( $dbh, $session['sessionID'], $transcript_data, $userid, $from_screen_name, $email, $deptid, $aspid, $mod_time, $phone ) ;
