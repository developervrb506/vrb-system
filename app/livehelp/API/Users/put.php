<?php
					...
					$sort_order )
	{
		...
		$use_in_tickets = database_mysql_quote( $use_in_tickets ) ;
		$sort_order = database_mysql_quote( $sort_order ) ;
		...
		if ( $deptinfo['deptID'] )
		{
			...
			$query = "REPLACE INTO chatdepartments VALUES ('$deptid', '$name', '$visible', '$save_transcripts', '$share_transcripts', '$exp_string', '$duration', '$show_que', '$email', $aspid, '$initiate_chat', '$email_transcripts', '$deptinfo[status_image_offline]', '$deptinfo[status_image_online]', '$deptinfo[status_image_away]', '$message_box', '$deptinfo[away_message]', '$greeting', '$use_in_tickets', '$sort_order')" ;
		}
		else
		{
			$message_box = database_mysql_quote( $LANG['MESSAGE_BOX_MESSAGE'] ) ;
			$query = "REPLACE INTO chatdepartments VALUES (0, '$name', '$visible', '$save_transcripts', '$share_transcripts', '$exp_string', '$duration', '$show_que', '$email', $aspid, '$initiate_chat', '$email_transcripts', '', '', '', '$message_box', '', '$greeting', '$use_in_tickets', '$sort_order')" ;
		}
		database_mysql_query( $dbh, $query ) ;
